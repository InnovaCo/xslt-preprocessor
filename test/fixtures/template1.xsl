...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:date="http://exslt.org/dates-and-times" xmlns:dyn="http://exslt.org/dynamic" xmlns:exsl="http://exslt.org/common" xmlns:common="http://exslt.org/common" xmlns:func="http://exslt.org/functions" xmlns:math="http://exslt.org/math" xmlns:random="http://exslt.org/random" xmlns:regexp="http://exslt.org/regular-expressions" xmlns:set="http://exslt.org/sets" xmlns:str="http://exslt.org/strings" extension-element-prefixes="date dyn exsl common func math random regexp set str">
	...
	<xsl:template match="/document">
		...
		<div><xsl:variable name="__x_attr_id"><xsl:if test="@id"><token><xsl:value-of select="@id"/></token></xsl:if></xsl:variable><xsl:call-template name="__x_attribute">
<xsl:with-param name="name" select="'id'"/><xsl:with-param name="nodeset" select="exsl:node-set($__x_attr_id)"/><xsl:with-param name="glue" select="' '"/></xsl:call-template><xsl:variable name="__x_attr_id2"><xsl:if test="contains(@id, 'foo')"><token><xsl:value-of select="@id"/></token></xsl:if></xsl:variable><xsl:call-template name="__x_attribute">
<xsl:with-param name="name" select="'id2'"/><xsl:with-param name="nodeset" select="exsl:node-set($__x_attr_id2)"/><xsl:with-param name="glue" select="' '"/></xsl:call-template><xsl:variable name="__x_attr_class"><xsl:if test="@mod1"><token><xsl:value-of select="@mod1"/></token></xsl:if>
<xsl:if test="@mod &gt; 2"><token><xsl:value-of select="@mod2"/></token></xsl:if></xsl:variable><xsl:call-template name="__x_attribute">
<xsl:with-param name="name" select="'class'"/><xsl:with-param name="nodeset" select="exsl:node-set($__x_attr_class)"/><xsl:with-param name="glue" select="' '"/></xsl:call-template>
		</div>

		<div>
			<xsl:call-template name="__x_add_attribute"><xsl:with-param name="name">extra1</xsl:with-param><xsl:with-param name="value"><xsl:if test="normalize-space(@id)"><xsl:value-of select="@id"/></xsl:if></xsl:with-param></xsl:call-template>
			...
			<xsl:variable name="__x_attribute_set"><a><xsl:value-of select="'test'"/></a>
<b><xsl:value-of select="@mod1"/></b></xsl:variable><xsl:call-template name="__x_add_attribute_set"><xsl:with-param name="nodeset" select="exsl:node-set($__x_attribute_set)"/><xsl:with-param name="prefix">data-</xsl:with-param></xsl:call-template>
		</div>
	...
	<xsl:with-param name="__x_to" select="number(@mod)"/>
